        ...
        try {
            if (slug === 'free-grammar-checker') {
                ...
                if (slug && (slug.includes('thesis') || slug.includes('dissertation') || slug.includes('essay-writing') || slug.includes('research') || slug.includes('case-study') || slug.includes('coursework')) && !slug.includes('outline') && !slug.includes('statement')) {
                    lengthInstruction = '\n- Length: aim for a detailed, academic piece of about 2,500 words that develops each argument fully.';
                }

                // The tool's role and formatting go to the server as instructions (sent as the
                // AI's system message); the prompt is only what the user typed.
                if (slug && content.title) {
                    instructions = `You are the "${content.title}" tool on an academic support website. What the tool does: ${content.desc}\n\nRespond only with the tool's output for the user's input: no greeting and no commentary about the task.\n\nFormatting:\n- Use clear headings (###) for the main sections.\n- Use bullet points or numbered lists for key points and steps.\n- Keep paragraphs short and the flow logical and professional.${lengthInstruction}`;
                }
                ...
                const res = await fetch(`${API_URL}/tools/process`, {
                    ...
                    body: JSON.stringify({ prompt: finalPrompt, ...(instructions && { instructions }) })
                });
